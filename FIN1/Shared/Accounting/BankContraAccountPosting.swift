...
enum BankContraAccount: String, Codable, CaseIterable {
    /// Holds the net app service charge portion before it is moved to revenue
    case appServiceChargeNet = "BANK-PS-NET"
    /// Holds the VAT portion of the app service charge before it is remitted
    case appServiceChargeVAT = "BANK-PS-VAT"

    var displayName: String {
        switch self {
        case .appServiceChargeNet:
            return "Bank Clearing – Service Charge NET"
        case .appServiceChargeVAT:
...
/// Helper structure returned when recording postings for an app service charge
...
protocol BankContraAccountPostingServiceProtocol: ServiceLifecycle {
    /// Records the contra account postings for an app service charge (net + VAT components)
