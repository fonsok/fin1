...
extension InvoiceService {
    ...
    func generateInvoicesForCompletedTrades(_ trades: [Trade]) async {
        guard !self.blocksLocalInvoiceGeneration else {
            print("ℹ️ InvoiceService: skip generateInvoicesForCompletedTrades — monetary server-only active (P3b)")
            return
        }
        ...
        await InvoiceLocalSynthesisGate.withPermitted {
            for trade in trades where trade.status == .completed {
                ...
                let existingInvoices = self.invoices.filter { $0.tradeId == trade.id }
                ...
                if !hasBuyInvoice {
                    ...
                    await self.addInvoice(buyInvoice)
                }
                ...
                for sellOrder in trade.sellOrders {
                    ...
                    if !invoiceExists {
                        ...
                        await self.addInvoice(sellInvoice)
                    }
                }
        ...
        print("📄 Invoice generation complete. Total invoices: \(self.invoices.count)")
    }
}
