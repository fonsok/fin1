...
final class PoolTradeParticipationService: PoolTradeParticipationServiceProtocol, ServiceLifecycle, @unchecked Sendable {
            ...
            do {
                ...
                await MainActor.run {
                    if let index = participations.firstIndex(where: { $0.id == participation.id }) {
                        self.participations[index] = savedParticipation
                    }
                }
    ...
    func getParticipations(forTradeId tradeId: String) -> [PoolTradeParticipation] {
        return self.participations.filter { $0.tradeId == tradeId }
    }

    func getParticipations(forInvestmentId investmentId: String) -> [PoolTradeParticipation] {
        return self.participations.filter { $0.investmentId == investmentId }
    }

    func getParticipations(forPoolReservationId poolReservationId: String) -> [PoolTradeParticipation] {
        return self.participations.filter { $0.poolReservationId == poolReservationId }
    }
    ...
    func distributeTradeProfit(
        ...
        await MainActor.run {
            let tradeParticipations = self.getParticipations(forTradeId: tradeId)
            ...
            for participation in tradeParticipations {
                ...
                if let index = participations.firstIndex(where: { $0.id == participation.id }) {
                    self.participations[index] = participation.withProfitShare(profitShare)
                }
            ...
            print(
                "✅ PoolTradeParticipationService: Distributed total profit of €\(String(format: "%.2f", totalDistributed)) to \(tradeParticipations.count) pools"
            )
    ...
    func getAccumulatedProfit(for investmentId: String) -> Double {
        let investmentParticipations = self.getParticipations(forInvestmentId: investmentId)
    ...
    func getAccumulatedProfit(forPoolReservationId poolReservationId: String) -> Double {
        let poolParticipations = self.getParticipations(forPoolReservationId: poolReservationId)
    ...
    func getAccumulatedProfit(forInvestmentReservationId investmentReservationId: String) -> Double {
        ...
        return self.getAccumulatedProfit(forPoolReservationId: investmentReservationId)
    }
}
