...
struct Investment: Identifiable, Codable, Sendable {
    ...
    var hasPartialSellRealization: Bool {
        let progress = self.tradeSellVolumeProgress ?? 0
        return self.partialSellCount > 0
            || self.realizedSellQuantity > 0
            || self.realizedSellAmount > 0
    ...
    var realizedSellSharePercentage: Double {
        ...
        return (self.realizedSellAmount / self.amount) * 100
    }
    ...
    var canonicalDisplayReference: String {
        ...
        return "INV-\(self.id.extractInvestmentNumber().uppercased())-\(split)"
    }
    ...
    func markAsCompleted(
        ...
        if let calculatedProfit = calculatedProfit, let calculatedReturn = calculatedReturn {
            ...
            profit = self.amount * baseReturnRate
        }

        // Calculate new current value (original amount + profit)
        let newCurrentValue = self.amount + profit
