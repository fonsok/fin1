import Foundation

enum LandingConstants {
    static let taglineText = "Meeting Point for\ncalculated\nHigh-Risk-High-Gain-Investors"
    static let getStartedButtonText = "Get Started"
    static let signInButtonText = "Sign In"
    //   static let appSubtitleText = "High-Risk-Growth App"
    static let appSubtitleText = "High-Risk-Growth-App"

    static let featureTexts: [(icon: String, text: String)] = [
        ("chart.bar.fill", "High-Risk-High-Gain"),
        ("percent", "Leveraged Products"),
        ("shield.fill", "Risk Management"),
        ("chart.line.uptrend.xyaxis", "Added Value Trading"),
        (
            "exclamationmark.shield",
            "High Risk High Gain:\nHohe Gewinne bedeuten auch hohes Risiko. Deshalb für Sie der folgende Risikohinweis:\nSollten Sie einen Verlust von bis zu 100 % Ihres eingesetzten Kapitals nicht ertragen können, löschen Sie diese App sofort."
        )
    ]
}
