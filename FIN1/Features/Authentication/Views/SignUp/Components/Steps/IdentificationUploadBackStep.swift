...
struct IdentificationUploadBackStep: View {
    let identificationType: IdentificationType
    @Binding var passportBackImage: UIImage?
    @Binding var idCardBackImage: UIImage?
    ...
    var body: some View {
        VStack(spacing: ResponsiveDesign.spacing(24)) {
            Text("Rückseite hochladen")
                .font(ResponsiveDesign.headlineFont())
                .fontWeight(.bold)
                .foregroundColor(AppTheme.fontColor)
                .multilineTextAlignment(.center)

            Text("Bitte laden Sie die Rückseite Ihres \(self.identificationType.displayName)es hoch")
            ...
            if let image = identificationType == .passport ? passportBackImage : idCardBackImage {
                    ...
                    if !self.appServices.testModeService.isTestModeEnabled {
                        Button(action: {
                            ...
                            if self.identificationType == .passport {
                                self.passportBackImage = (self.appServices.testModeService as? TestModeService)?.samplePassportImage
                            } else {
                                self.idCardBackImage = (self.appServices.testModeService as? TestModeService)?.sampleIDCardImage
                            }
                        }) {
                            HStack {
                                Image(systemName: "testtube.2")
...
#Preview {
    IdentificationUploadBackStep(
        ...
        passportBackImage: .constant(nil),
        idCardBackImage: .constant(nil)
    )
