...
/**
 * Embedded sellOrder on Trade — mirrors buyOrder snapshot pattern.
 */
function buildSellOrderSnapshotFromOrder(order) {
  ...
  const executedAt = order.get('executedAt') instanceof Date
    ...
    : createdAt;
  const confirmedAt = order.get('confirmedAt') instanceof Date
    ? order.get('confirmedAt').toISOString()
    : null;
  ...
  return {
    ...
    originalHoldingId: order.get('originalHoldingId') || null,
  };
}

module.exports = {
  buildSellOrderSnapshotFromOrder,
};
