#!/usr/bin/env bash
# E2E smoke: retail role immutable after signup (server + saveOnboardingProgress + PUT /users).
set -euo pipefail
...
if [[ "${SMOKE_ON_SERVER:-1}" == "1" ]]; then
  rsync -az "$SCRIPT_DIR/smoke-signup-role-immutability.js" \
    "${REMOTE_USER}@${REMOTE_HOST}:~/fin1-server/scripts/"
  # shellcheck disable=SC2029
  ssh "${REMOTE_USER}@${REMOTE_HOST}" \
    "cd ~/fin1-server && PARSE_URL='http://127.0.0.1:1338/parse' \
      SMOKE_CLEANUP='${SMOKE_CLEANUP:-1}' \
      BA_PASSWORD='${BA_PASSWORD:-}' \
      node scripts/smoke-signup-role-immutability.js"
  exit $?
fi
...
exec node "$SCRIPT_DIR/smoke-signup-role-immutability.js" "$@"
