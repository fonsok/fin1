#!/usr/bin/env bash
# ===========================================================================
# Technischer Admin (Parse-Rolle admin) — gleiche Portal-Login-URL wie Finance Admin
#
# Vorgesehene Demo-Mail: admin@fin1.de (siehe admin-portal/src/constants/portalLogin.ts).
#
# Auf dem Parse-Host ausführen:
#   BA_PASSWORD='…' bash scripts/create-tech-admin.sh
#   — oder BA_PASSWORD in scripts/.env.server (gitignored), siehe scripts/.env.server.example
#
# Andere E-Mail / Namen:
#   BA_EMAIL="ops@fin1.de" BA_PASSWORD="<IhrPasswort>" BA_FIRST="Ops" BA_LAST="Admin" bash scripts/create-tech-admin.sh
#
...
if command -v docker >/dev/null 2>&1 && docker ps --format '{{.Names}}' 2>/dev/null | grep -q parse-server; then
  echo "Erstelle / setze Technischen Admin: $BA_EMAIL (Rolle: admin)"
  docker exec \
    -e BA_EMAIL="$BA_EMAIL" \
  ...
  echo "  cd ~/fin1-server && BA_PASSWORD='<IhrPasswort>' bash scripts/create-tech-admin.sh"
  exit 1
fi
