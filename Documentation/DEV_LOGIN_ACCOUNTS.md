...
| `company1-draft@test.com` | `investor` (Firma) | KYB Wizard offen — `seedCompanyTestUsers` |
| `company1-pending@test.com` | `investor` (Firma) | KYB eingereicht, Investing gesperrt bis Admin-Freigabe |
| `company1-approved@test.com` | `investor` (Firma) | KYB freigegeben, Investing erlaubt (sofern RC/Gates passen) |

**Company-Testuser anlegen:** `node backend/scripts/seed-company-test-users.js` (Master Key aus `scripts/.env.server` oder Parse-`.env`). iOS DEBUG: Landing → **Company Investors (KYB)**.
