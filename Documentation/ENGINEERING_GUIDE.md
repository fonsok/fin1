...
- **Onboarding shell / defer (2026-06):** `AuthenticationView` shows `MainTabView` only when `onboardingCompleted`; `AppRootContent` skips retail background sync until then; `SLAMonitoringService.canRunMonitoring` is staff-only (`admin` / `customerService`). Gate logic split across `AuthenticationView+LegalConsent` / `+Onboarding` / `+AuthenticatedContent` (file-size CI). `discoverTraders` uses `profileDisplayName.js` for safe `displayName`. Ops smoke: `./scripts/run-smoke-signup-role-immutability.sh`.
- **Post-onboarding Re-Consent (2026-06):** Nach Device-Gate prüft iOS `getUserMe.requiredReConsents` (`ReConsentModalView`); Backend `getRequiredReConsents` + `productAccessGate` Version-Drift. **`ReConsentViewModel`** als `@State` in `AuthenticationView` (Factory `makeReConsentViewModel()`), nicht `@ObservedObject` + Inline-Init — verhindert Spinner-Loop nach Login. Abnahme: [`Documentation/RELEASE_ABNAHME_RE_CONSENT.md`](RELEASE_ABNAHME_RE_CONSENT.md), Release: [`Documentation/RELEASE_IOS_2026-06-29_RECONSENT_DEPOT.md`](RELEASE_IOS_2026-06-29_RECONSENT_DEPOT.md).
- **Trader buy-order wiring (2026-06):** `BuyOrderViewModelFactory` + `BuyOrderDependencies` zentralisieren Placement/Service-Injection; Depot-KAUFEN nutzt `.buyOrderSheet` wie Suche/Watchlist. Offenes Refactoring (Backlog): L2.2–L2.4 Presentation-State — siehe Release-Notiz.
