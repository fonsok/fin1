...
- **Reparatur (2026-05, Gegenrichtung):** Kontoauszug-Links ok, aber **Notifications → Documents** leer/unvollständig (Investor ohne CB, Trader ohne Gutschrift). Ursachen: fehlendes Settlement-Document-Merge nach Backend-Abschluss, 24h-Filter im Documents-Tab, `userId`-Alias; Backend `getInvestorCollectionBills` mit `collectLedgerUserIdCandidates`. Details: `Documentation/ACCOUNT_STATEMENT_ARCHITECTURE.md` → *Notifications → Documents*.
...
- **Inbox (SSOT):** Cloud Function `getUserDocumentInbox` — ein Request, `collectLedgerUserIdCandidates`, gleiche Filter wie iOS `DocumentInboxPolicy`. Client: `DocumentService.refreshUserDocumentInbox` (45s TTL, Retry, Legacy-Fallback), `applyInboxSnapshot` erhält Deep-Links. Invalidation: `Notification.Name.userDocumentInboxShouldRefresh` (Settlement, Investment). Tab **Documents** ohne 24h-Lese-Fenster.
