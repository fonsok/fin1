...
- **`getRequiredReConsents`**: auth required, params `{language?}` (Default `de`) → `{ required: [{ consentType, documentType, activeVersion, userVersion, blocking, requiresScrollToAccept }] }` — SSOT `resolveRequiredReConsents` (`legalConsentUserSync.js`). Eintrag nur wenn `_User.*Version` **gesetzt** und **älter** als aktive `TermsContent`-Version (Legacy-Nutzer ohne Versionsfeld: **Grandfather**, kein erzwungenes Re-Consent). `consentType`: `terms_of_service`, `privacy_policy`, `trader_agreement`, `investor_agreement`.

**Produkt-Zugriff (serverseitig):** `utils/productAccessGate.js` — `assertProductAccessEligible(user)` vor kritischen Trading-/Investment-Cloud-Functions (`placeOrder`, `investmentCreateSplits`, …); erfordert `onboardingCompleted === true`, `acceptedTerms` **und** `acceptedPrivacyPolicy`, passende Rollenvereinbarung (Retail), **kein blockierender Eintrag in `resolveRequiredReConsents`** (Versions-Drift → `OPERATION_FORBIDDEN` mit spezifischer Meldung, z. B. *Terms of Service must be re-accepted (version X required).*), sowie für **`accountType == company`** `companyKybStatus === approved`.
