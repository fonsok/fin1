...
3. **Server-Orchestrierung — Cloud Function `executePairedBuy`** (`functions/trading.js`, nur Rolle **trader**):
   ...
   - **Sequentielles Speichern der Beine** (kein paralleles `saveAll` — `beforeSave`/`orderNumber` würde sonst racen): bei Fehler nach Teilerfolg werden angelegte Orders per **`destroyAll`** bereinigt, `PairedExecution` → `ABORTED` (best-effort Compensation). Implementierung: `functions/tradingPairedBuyExecution.js`; Regel: `.cursor/rules/parse-cloud.md` § Multi-Leg Orders.
   ...
   - **Gekoppelte Order-Status (100 % Paired-SSOT):** **`advancePairedOrderStatus`** koppelt Zwischenstatus (`submitted` → `confirmed` → `completed`; `suspended` UI-only, Catch-up in Commit). **`commitPairedBuyExecution`** (bevorzugt, iOS) = Settlement bei `executed` + optional **`postDisplayStatus`** (`completed` als leichtgewichtiger Post-Only-Pfad nach `effectsApplied`). Legacy: **`finalizePairedBuyExecution`**. **Stufe B Rest (Skalierung):** **`pairedBuySettlementQueue`** — Redis-Lock pro `pairExecutionId` + globales Semaphor für Finalize (`PAIRED_BUY_FINALIZE_MAX_CONCURRENT`, default 4); **`distributedSemaphore`** + **`redisClient`** (Fallback in-process ohne `REDIS_URL`). Pool-Mirror: **`poolActivationLimiter`** über dasselbe Semaphor (`POOL_MIRROR_MAX_CONCURRENT_ACTIVATIONS`, default 6) + Investor-Cap (`POOL_MIRROR_MAX_INVESTORS_PER_TRADE`, default 2000). Skalierung: `CREDIT_NOTE_INLINE_BREAKDOWN_MAX` / `SUMMARY_REPORT_INLINE_PARTICIPATIONS_MAX` (default 50), `SETTLEMENT_PARTICIPATION_BATCH_SIZE` (default 25), Admin-API `getSummaryReportTradeParticipationsPage`. Admin **`getPairedOrderStatusIntegrityStatus`** inkl. `pairedBuySettlementQueue`-Stats. **CI:** `.github/workflows/paired-order-status-integrity-monitor.yml` → `scripts/monitor-paired-order-status-integrity.js` (Mo 05:55 UTC). **Load-Probe (manuell):** `scripts/load-test-paired-buy-parallel.sh` / Workflow `paired-buy-load-test.yml`. **Indexes:** `backend/scripts/ensure-paired-execution-indexes.js` (rsync via Deploy-Skript nach `~/fin1-server/scripts/`).
...
- **`OrderLifecycleCoordinator+Settlement.generateCreditNoteIfCommissionExists()`**: Tries `fetchTradeSettlement()` first for authoritative commission/gross profit; falls back to local `InvestorGrossProfitService` + `CommissionCalculationService` if backend unavailable.
