...
- **`commission_debit` (Legacy):** historische Zeilen bleiben in Parse `AccountStatement` (Admin **Ledger GoB**), erscheinen in der **Kundensicht nicht**, wenn für denselben Trade/Investment bereits `investment_return` (Überweisungsbetrag) gebucht ist — Provision ist dort enthalten (`transferAmount = netSellAmount − commission`). Neue Settlements buchen Provision nur noch GL-intern (`bookInvestorCommissionClearingGL`).
