...
3. **Mittel (Parse-Image + GHCR):** Workflow **`.github/workflows/parse-server-docker-build.yml`** baut mit **Buildx** + **GHA-Cache**; auf **Push / `workflow_dispatch`** auf **`main`/`master`** (nicht auf PRs) wird nach **GHCR** gepusht: **`ghcr.io/<github-org-klein>/fin1-parse-server:<sha>`** und **`:<branch>`** (`fin1-parse-server` immer kleingeschrieben). Lokal wie CI-Build: **`./scripts/ci-build-parse-server-docker.sh`**. Auf dem Server optional **`docker-compose.parse-server-ghcr.yml`** mergen (`build: !reset null` + **`FIN1_PARSE_SERVER_IMAGE`** in **`~/fin1-server/.env`**) und **`docker compose … pull`** / **`up`** — Schritt-für-Schritt im Runbook **§ 8.1**. Cloud-Code **im Image** vs. Host-Volume bleibt euer bewusster Schalter.  
